;; disable all the bindings that come with magit
;; since they confuse me as a vim user (they clobber motion commands)
;; define bindings for the ones I care about (WIP)

;; instead of starting with a (make-keymap) we start by copying the evil-motion-state-map
;;FIXME no evil-leader ... also define "," as evil-leader--default-map to allow leader commands to work in magit modes
...
(defun set-evil-magit-bindings ()
  (defvar magit-mode-map
    (let ((map (copy-keymap evil-motion-state-map)))
      ...
      (define-key map (kbd "^") 'magit-section-parent)
      ...
      ;; (define-key map (kbd "???") 'magit-refresh)
      ;; (define-key map (kbd "???") 'magit-refresh-all)
                                        ;(define-key map (kbd "?") 'magit-key-mode-popup-dispatch)
      (define-key map (kbd ",,gc") 'magit-git-command)
      ...
      (define-key map (kbd ",,ps") 'magit-push-current-to-upstream)
